...
public class ServerDeathmatch : MonoBehaviour {
    ...
    private void JoinPlayer(Packet packet)
    {
        ...
        ClientInfo clientInfo = new ClientInfo(clientUsername, endPoint, -1);
    ...
    private void BroadcastNewPlayer(int newPlayerId, Vector3 position, Vector3 rotation)
    {
        ...
        foreach (var id in clients.Keys)
        {
            if (!clients[id].disconnected)
            {
                ...
                NewPlayerBroadcastEvent newPlayerEvent = new NewPlayerBroadcastEvent(newPlayerId, newPlayer, serverTime, id, -1);
